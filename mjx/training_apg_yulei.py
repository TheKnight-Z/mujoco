...
def render_rollout(reset_fn, step_fn, 
    ...
    save_video(env.render(rollout, camera=camera),
               filename=filename, fps=float(1.0 / (env.dt*render_every)))
    ...
    default_config = config_dict.ConfigDict(
        dict(
            scales=config_dict.ConfigDict(
              dict(
                min_reference_tracking = 0.0, # to equalize the magnitude
...
class TrotAnymal(PipelineEnv):
  ...
  def _get_obs(self, qpos: jax.Array, x: Transform, xd: Motion,
    ...
    obs_list.append(kin_ref[7:]) # First 7 indicies are fixed # (12)
...
render_rollout(
  ...
  demo_env,
  filename='output_trot_apg_noangle_track_nolastact_wvel.mp4',
  n_steps=200,
  seed=1
)

model_path = '/home/yulei/mujoco/mjx/trotting_2hz_policy_apg_noangle_track_nolastact_wvel'
