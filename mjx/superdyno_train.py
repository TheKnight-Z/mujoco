...
def compute_tracking_loss_from_state(pred_state: jnp.ndarray, target_state: jnp.ndarray):
    ...
    return _mse_pos ,_mse_rot, _mse_vel, _mse_ang
...
def quat_integrate(quat: jnp.ndarray, ang_vel: jnp.ndarray, dt: float) -> jnp.ndarray:
    ...
    delta_q = 0.5 * dt * jax.vmap(jax.vmap(math.quat_mul, in_axes=(0, 0)), in_axes=(0, 0))(omega, quat)
    new_q = quat + delta_q
...
def make_world_predict_fn(
    ...
    def world_predict(params, deterministic: bool = False):
        def predict(states: jnp.ndarray, actions: types.Action):
            ...
            B = states.shape[0]
            new_vel_delta = new_part_state[:, :3*13].reshape(-1, 13, 3)
            new_ang_delta = new_part_state[:, 3*13:].reshape(-1, 13, 3)
            ...
            new_xd_vel = last_xd_vel + rotate_vec_batch(base_rot, new_vel_delta)
            new_xd_ang = last_xd_ang + rotate_vec_batch(base_rot, new_ang_delta)
...
def superdyno_train(
    ...
    def policy_model_loss(policy_params, normalizer_policy_params, world_params, normalizer_world_params, policy_data, key):
        f = functools.partial(
            ...
            world_model=world_predict((normalizer_world_params, world_params))
        )
        ...
        pos_loss, rot_loss, vel_loss, ang_loss = compute_tracking_loss_from_state(pred_states, policy_data['next_state'][:,:-1])
        
        policy_loss = pos_loss + rot_loss + vel_loss + ang_loss
        
        return policy_loss, (pos_loss, rot_loss, vel_loss, ang_loss)

    def world_model_loss(world_params, normalizer_world_params, world_data, key):
        # breakpoint()
        f = functools.partial(
            world_model_training_step, world_model=world_predict((normalizer_world_params, world_params))
        )
        ...
        # breakpoint()
        # TODO: calculate the loss with pred_states and reference states
        pos_loss, rot_loss, vel_loss, ang_loss = compute_tracking_loss_from_state(pred_states, world_data['next_state'][:,:-1])
        
        world_loss = pos_loss + rot_loss + vel_loss + ang_loss
        
        return world_loss, (pos_loss, rot_loss, vel_loss, ang_loss)
    ...
    def minibatch_world_step(optimizer_world_state, normalizer_params, world_params, key, world_data):
        ...
        grad, (pos_loss, rot_loss, vel_loss, ang_loss ) = loss_grad_world(
        ...
        metrics = {
            ...
            'world_pos_loss': pos_loss,
            'world_rot_loss': rot_loss,
            'world_vel_loss': vel_loss,
            'world_ang_loss': ang_loss,
    ...
    def minibatch_policy_step(optimizer_policy_state, normalizer_params, policy_params, world_params, normalizer_world_params, key, policy_data):
        ...
        grad, (pos_loss, rot_loss, vel_loss, ang_loss) = loss_grad_policy(
        ...
        metrics = {
            ...
            'policy_pos_loss': pos_loss,
            'policy_rot_loss': rot_loss,
            'policy_vel_loss': vel_loss,
            'policy_ang_loss': ang_loss,
    ...
    def training_epoch(
        ...
        for i in range(batched['state'].shape[0]):
            ...
            metrics['world_pos_loss'] += metrics_w['world_pos_loss'] / world_updates_per_epoch
            metrics['world_rot_loss'] += metrics_w['world_rot_loss'] / world_updates_per_epoch
            metrics['world_vel_loss'] += metrics_w['world_vel_loss'] / world_updates_per_epoch
            metrics['world_ang_loss'] += metrics_w['world_ang_loss'] / world_updates_per_epoch
        ...
        for i in range(batched['state'].shape[0]):
            ...
            metrics['policy_pos_loss'] += metrics_p['policy_pos_loss'] / policy_updates_per_epoch
            metrics['policy_rot_loss'] += metrics_p['policy_rot_loss'] / policy_updates_per_epoch
            metrics['policy_vel_loss'] += metrics_p['policy_vel_loss'] / policy_updates_per_epoch
            metrics['policy_ang_loss'] += metrics_p['policy_ang_loss'] / policy_updates_per_epoch
    ...
    def training_epoch_with_timing(
        ...
        collect_env_data = jax.pmap(collect_env_data, axis_name=_PMAP_AXIS_NAME)
        state_h, transition_data, normalizer_policy_params = collect_env_data(
            training_policy_state.network_params,
            training_policy_state.normalizer_params,
            env_state,
        ...
        # then sample data from the buffer for policy/world training
        # breakpoint()
        transition_data = {k: v[0] for k, v in transition_data.items()}
        ...
        filtered_transition_data['done'] = filtered_transition_data['done'].at[-1, :].set(1)
        T,  B, _ = filtered_transition_data['done'].shape
        filtered_transition_data = { k: v.reshape( B * T, -1) for k, v in filtered_transition_data.items() }
        jax.debug.print("added_transition_data:{}", B*T)
        # breakpoint()
        # breakpoint()
